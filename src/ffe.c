...
/* $Id: ffe.c,v 1.87 2010-03-14 12:33:34 timo Exp $ */
...
/* adds an item to expression list, returns a pointer to list head 
 */
static struct expr_list *
add_expression_to_list(struct expr_list *e, char *value)
{
    struct expr_list *start = e;
...
static struct expr_list *
read_expression_file(char *file)
{
    struct expr_list *e;
    FILE *fp;
    int ccount;
    ...
    do
    {
        ...
        if (ccount != -1)
        {
            if(ccount) ccount--;
            line[ccount] = 0;
            e = add_expression_to_list(e,line);
        }
    }
    while(ccount != -1);
    fclose(fp);
...
add_expression(char *optarg)
{
    ...
    *op_pos = 0;
    ...
    if(strstr(op_pos,"file:") == op_pos)
    {
        ...
        e->el = read_expression_file(value_file);
        if(e->el == NULL) panic("No values in file",value_file,NULL);
        free(value_file);
    } else
    {
        e->el = add_expression_to_list(e->el,op_pos);
    }
    e->found = 0;
    e->op = op;
#ifdef HAVE_REGEX
    if(e->op == OP_REQEXP)
    {
        ...
        while(el != NULL)
        {
            rc = regcomp(&el->reg,el->value,REG_EXTENDED | REG_NOSUB);
            if(rc)
            {
                buflen = regerror(rc,&el->reg,NULL,0);
                errbuf = xmalloc(buflen + 1);
                regerror(rc,&el->reg,errbuf,buflen);
                panic("Error in regular expression",el->value,errbuf);
            }
            el = el->next;
        }
    }
